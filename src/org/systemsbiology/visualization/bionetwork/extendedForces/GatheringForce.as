package org.systemsbiology.visualization.bionetwork.extendedForces
{
	...
	public class GatheringForce implements IForce
	{
		 ...
		 */
		public function GatheringForce(g:Number=-1,rest : Number =10, nodes:Array = null, max:Number=200, min:Number=2,
		...
		public function apply(sim:Simulation):void
		{
        	...
        	{
        		// get the tree bounds
				boundsFromNodes(_datanodes);
				
        		for (var i:uint = 0; i<_datanodes.length; ++i) {
	        	
	        		insert(_datanodes[i].props.particle as Particle, _root, _x1, _y1, _x2, _y2);
	        	}	
	        	...
	        	for (i=0; i<_datanodes.length; ++i) {
	        		forces(_datanodes[i].props.particle as Particle, _root, _x1, _y1, _x2, _y2);
	        	}
        	}
        	else
        	{
		...
		private function boundsFromNodes(_nodes:Array):void
		{
			var p:Particle, dx:Number, dy:Number;
			_x1 = _y1 = Number.MAX_VALUE;
			_x2 = _y2 = Number.MIN_VALUE;
			...
			for (var i:uint = 0; i<_nodes.length; ++i) {
				p = _nodes[i].props.particle as Particle;
				if (p.x < _x1) _x1 = p.x;
				if (p.y < _y1) _y1 = p.y;
				if (p.x > _x2) _x2 = p.x;
				if (p.y > _y2) _y2 = p.y;
